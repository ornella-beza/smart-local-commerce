    ...
    <Card className="group relative overflow-hidden hover:shadow-lg transition-shadow">
      <CardContent className="p-6">
        <div className="relative mb-4">
          {badge && (
            <Badge className={`absolute top-2 left-2 ${getBadgeClassName()} px-4 py-1.5 rounded-none text-xs font-semibold whitespace-nowrap uppercase`}>
              {badge.text}
            </Badge>
          )}
          <button 
        ...
        {category && (
          <p className="text-muted-foreground uppercase tracking-wider mb-2" style={{ fontSize: '0.75rem', fontWeight: 500 }}>
            {category}
          </p>
        )}
        
        <h3 className="font-semibold mb-3 body-normal">{title}</h3>
        ...
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-2xl font-bold">RWF {price.toFixed(0)}</span>
        </div>
        ...
        {colors && colors.length > 0 && (
          <div className="flex items-center gap-2">
            {colors.map((color, index) => (
              <button
                key={index}
                className="w-5 h-5 rounded-full border-2 border-transparent hover:border-gray-300 transition-colors"
