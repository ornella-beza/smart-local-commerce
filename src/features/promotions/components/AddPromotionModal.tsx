...
export function AddPromotionModal({ isOpen, onClose, onSuccess }: AddPromotionModalProps) {
      ...
      const fetchData = async () => {
        try {
          const [shopsData, categoriesData] = await Promise.all([
            shopService.getMyShops().catch(() => shopService.getShops()), // Try getMyShops first, fallback to all shops
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const result = await promotionsService.createPromotion(submitFormData);
      ...
      <Card className="w-full max-w-2xl max-h-[90vh] overflow-y-auto m-4 p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4" noValidate>
          ...
          <div>
            ...
            <Select
              value={formData.shop}
              onValueChange={(value: string) => setFormData((prev) => ({ ...prev, shop: value }))}
              required
            >
              <SelectTrigger className="w-full">
