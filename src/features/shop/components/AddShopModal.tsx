...
export function AddShopModal({ isOpen, onClose, onSuccess }: AddShopModalProps) {
  ...
  useEffect(() => {
    if (isOpen) {
      // Pre-fill email with user's email when modal opens
      setFormData({
        name: '',
        description: '',
        location: '',
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    e.stopPropagation();
    console.log('=== SHOP FORM SUBMISSION STARTED ===');
    ...
    try {
      ...
      console.log('Validation passed, proceeding with shop creation...');
      ...
      if (imageFile) {
        ...
        console.log('Image file selected for shop');
      }
      ...
      const result = await shopService.createShop(submitFormData);
      ...
      const errorMessage = err.message || 'Failed to create shop. Please try again.';
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                ...
                {user?.email && <span className="text-xs text-gray-500 ml-2">(Your account email)</span>}
              </label>
              <Input
                type="email"
                name="email"
                ...
                readOnly={!!user?.email}
                title={user?.email ? "This will be set to your account email" : ""}
              />
            </div>
          </div>
